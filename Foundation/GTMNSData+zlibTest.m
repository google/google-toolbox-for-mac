...
- (void)testBoundaryValues {
  ...
  // Ensure passing nil/NULL doesn't crash, even though it shouldn't be done.
  id passNil = nil;
  void *passNULL = NULL;

  // bogus args to start
  NSError *error = nil;
  XCTAssertNil([NSData gtm_dataByDeflatingData:passNil error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByDeflatingBytes:passNULL length:666 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByDeflatingBytes:[data bytes] length:0 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByGzippingData:passNil error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByGzippingBytes:passNULL length:666 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByGzippingBytes:[data bytes] length:0 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByInflatingData:passNil error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByInflatingBytes:passNULL length:666 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByInflatingBytes:[data bytes] length:0 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByRawDeflatingData:passNil error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByRawDeflatingBytes:passNULL length:666 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByRawDeflatingBytes:[data bytes] length:0 error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByRawInflatingData:passNil error:&error]);
  XCTAssertNil(error);
  error = nil;
  XCTAssertNil([NSData gtm_dataByRawInflatingBytes:passNULL length:666 error:&error]);
  XCTAssertNil(error);
  error = nil;
